	     ...
	     #array of paiement
	      ...
	      while (($row = ibase_fetch_row($result))) {
	 		...
	 		$nombreLigne++;
          	...
          	#$prochainPaiement['date']= date('d-m-y', strtotime(trim($row[1])));
			...
			else if (($mois > $month)||($annee > $year)) {
				
				$variableTest = 2;
			}
			else if (($jour > $day) && ($mois == $month) && ($annee == $year)) {
				
				$variableTest = 3;
			}

			if (($variableTest == 1)||($variableTest == 2)||($variableTest == 3)) {
